...
%   rvazdev-ex, "Gamma Operators for Pochhammer Kernels"
...
\begin{definition}[Palindromic of degree $n$]
  ...
  \leanok
  $Q \in \RR[X]$ is \emph{palindromic of formal degree $n$} when its
  natural degree is at most $n$ and $[X^{n-k}]Q = [X^k]Q$ for all
  $0 \leq k \leq n$.
\end{definition}
...
\begin{definition}[$\gamma$-polynomial]
  ...
  \leanok
  $\Gpoly{m}{\gamma} = \sum_{j=0}^{m} \gamma_j\, X^j$,
  the ordinary generating polynomial of the $\gamma$-coefficients.
\end{definition}
...
\begin{lemma}[Hadamard closure / Aissen--Schoenberg--Whitney]
  ...
  \leanok
  Let $A, B \in \RR[X]$ have non-negative coefficients up to degree $n$
  and only real, non-positive zeros.  Then the truncated Hadamard product
  $\operatorname{hadamard}(n, A, B)$ has only real, non-positive zeros.
\end{lemma}
...
\begin{lemma}[Schur--Maló: $\Lop$ preserves non-positive-rooted]
  ...
  $\ell \in \NN$ and $\alpha > 0$, the Schur transform
...
\begin{lemma}[$\gamma$-representation]
  ...
  Let $Q$ be palindromic of formal degree $n = 2m + \varepsilon$
  ($\varepsilon \in \{0,1\}$) with only real, strictly negative zeros.
  Then there exist coefficients $\gamma_0, \ldots, \gamma_m$ such that
  \(
    Q \;=\; \sum_{j=0}^{m} \gamma_j\, \gammabasis{n}{j}.
  \)
  Moreover the $\gamma$-polynomial
  $\sum_{j=0}^{m}\gamma_jX^j$ has only real, non-positive zeros.
\end{lemma}
...
\begin{theorem}[Theorem~1 --- $\Pn$ preserves real-negative-rooted families]
  ...
  negative zeros and degree exactly $n$.  Then
  $\Pn(X; f)$ has only real, strictly negative zeros.
\end{theorem}

\begin{proof}
  \leanok
  \uses{def:determinantPolynomial, lem:hadamard_closure,
        lem:schur_preserves_nonpos,
        lem:gamma_representation,
        ...
        lem:rung_action_on_gamma_basis,
        thm:ladder_formula}
  ...
  \begin{enumerate}
    ...
    \item Apply the rung-action formula
          (Lemma~\ref{lem:rung_action_on_gamma_basis})
...
\begin{definition}[$\lambda_{s,q}$ coefficients]
  ...
  coefficient
  \[
    \lcoef{s}{0}=\frac{1}{(s!)^2}, \qquad
    \lcoef{s}{q}=\frac{2(-1)^q}{(s-q)!(s+q)!}\quad(q>0).
  \]
\end{definition}
...
\begin{definition}[Rung kernel $K^{[s,\mu]}_{n,k}$]
  ...
  \leanok
  $\Krung{n}{k}{s}{\mu}(X)
   = \sum_{q=0}^{s}\lcoef{s}{q}\,
      \Poch{X+\mu+q}{k}\,\Poch{X+\mu-q}{n-k}$.
\end{definition}
...
\begin{definition}[$A_{s,q}$ triangle]
  ...
  \leanok
  $\Atriangle{s}{q}$ is the unsigned normalized triangle from OEIS
  A380113: $\Atriangle{0}{0}=1$,
  $\Atriangle{s}{0}=\frac12\binom{2s}{s}$ for $s\geq1$, and
  $\Atriangle{s}{q}=\binom{2s}{s-q}$ for $1\leq q\leq s$.
\end{definition}
...
\begin{lemma}[Lemma~5 --- rung action on the $\gamma$-basis]
  ...
  \leanok
  \uses{def:rungOperator, def:gammaBasis, lem:central_difference_identity}
...
\begin{theorem}[Theorem~2 --- ladder formula]
  ...
  \leanok
  \uses{lem:rung_action_on_gamma_basis, def:rungOperator, def:gammaBasis}
  Statement of the paper's ladder formula: applied to a $\gamma$-expansion,
  the rung operator $T_{s,\mu}^{(n)}$ factors as a Pochhammer prefactor
  times the Schur transform of
  $4^{m-s}\mathrm{G}^{(s)}(t/4)$, evaluated after the shift $X\mapsto X+\mu$.
  (Full statement in the source.)
\end{theorem}
...
\begin{corollary}[Corollary~1 --- original determinant kernel]
  ...
  On palindromic negative-rooted inputs, the original determinant-kernel
  operator produces a polynomial that is either zero or has only real,
  non-positive zeros.
\end{corollary}
...
\begin{definition}[Centered kernel $K^{a,c}_{n,k}$]
  ...
  \leanok
  $\Kcent{n}{k}{a}{c}(X)
    = \Poch{X+a}{k}\,\Poch{X-a}{n-k}
      - \Poch{X+c}{k}\,\Poch{X-c}{n-k}$.
\end{definition}
...
\begin{definition}[Universally admissible]
  ...
  $T^{a,c}_n$ sends every palindromic polynomial $Q$ with only real,
  strictly negative zeros to a polynomial that is either zero or has only
