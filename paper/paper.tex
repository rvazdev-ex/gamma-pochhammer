...
By \Cref{lem:gamma}, \(\Gamma\) has only real nonpositive zeros. Rolle's theorem implies that \(\Gamma^{(s)}\) also has only real nonpositive zeros, unless it is zero. Scaling \(t\mapsto t/4\) preserves this property. \Cref{lem:schur} gives that the \(\mathcal S\)-factor in \eqref{eq:ladder} has only real nonpositive zeros in \(w\), and the prefactor \((w+s)_{m-s+\varepsilon}\) also has only real nonpositive zeros in \(w\). Therefore all zeros satisfy \(w\leq0\), equivalently \(z\leq-\mu\). The conclusions follow.
\end{proof}
